...
A focused Store capability is added only when a proven caller needs a stream, callback, resource scope, cleanup rule, result lifecycle, or engine guarantee that lower-tier contracts cannot preserve and at least one adapter can implement it. Driver-independent primitive contracts belong in `@commissary/store`; driver- or ORM-specific contracts remain with their adapter. Store has no speculative optional capability methods.
