...
function artifactId(value: string): ArtifactId {
  return ArtifactId.decode(value);
}
...
async function run(
  ...
  const client = {
    ...coreClient,
    async run(input: AgentCreateRunInput) {
      const submission = await coreClient.createRun(input);
      if (submission.type !== "accepted") {
        return submission;
      }
      return (await coreClient.execute(submission.runId)).result;
    },
    async stream(input: AgentCreateRunInput) {
      const submission = await coreClient.createRun(input);
      if (submission.type !== "accepted") {
        return submission;
      }
...
it("exposes Agent operations as native Effects", async () => {
  ...
  const submission = await Effect.runPromise(
    fixture.effectClient.createRun({
...
it("streams canonical reasoning deltas as Model Events", async () => {
  ...
  const fixture = await run(fake);
  const deltas: string[] = [];
  const unsubscribe = fixture.client.on(HookPoints.onExecutionEvent, ({ event }) => {
  ...
  const artifactStore: ArtifactStore = {
    ...
    write(content) {
      ...
      return Promise.resolve({
        id: ArtifactId.decode("artifact-1"),
...
it("reports Artifact Store read failures as specific store errors", async () => {
  ...
  const execution = fixture.client.run({
    ...
    message: {
      role: "user",
      content: [
        Content.file({ id: ArtifactId.decode("missing-artifact"), mediaType: "image/png" }),
      ],
    },
  });
